  ...
  void SGM::compute_path_cost(int direction_y, int direction_x, int cur_y, int cur_x, int cur_path)
  {
    ...
    else
    {
      ...
      for(unsigned int d = 0; d < disparity_range_;d++){
          path_cost_[cur_path][cur_y][cur_x][d] = cost_[cur_y][cur_x][d];
          prev_cost = cost_[cur_y][cur_x][d];
          if(d == 0) {
              small_penalty_cost &= cost_[cur_y][cur_x][d + 1] + p1_;
          }
          else if(d == disparity_range_-1)
              small_penalty_cost &= cost_[cur_y][cur_x][d-1] + p1_;
          else{
              if(cost_[cur_y][cur_x][d+1]>cost_[cur_y][cur_x][d-1])
                  small_penalty_cost &= cost_[cur_y][cur_x][d-1] + p1_;
              else
                  small_penalty_cost &= cost_[cur_y][cur_x][d+1] + p1_;
          }
          ...
          for(unsigned int dd = 0; dd < disparity_range_;dd++){
              if(dd - d >= 2 || dd - d <= -2){
                  if (best_prev_cost > cost_[cur_y][cur_x][dd])
                      best_prev_cost = cost_[cur_y][cur_x][dd];
              }
          }
          big_penalty_cost = best_prev_cost+p2_;
          unsigned long temp = min(prev_cost,small_penalty_cost);
          path_cost_[cur_path][cur_y][cur_x][d] += min(temp,big_penalty_cost);
      }
    }
